...
import eufy, { CaptchaOptions, EufySecurity, P2PClientProtocol, P2PConnectionType, PropertyName } from 'eufy-security-client';
...
class EufyCamera extends ScryptedDeviceBase implements VideoCamera, MotionSensor {
  ...
  setupMotionDetection() {
    const handle = (device: eufy.Device, state: boolean) => {
      ...
      this.console.debug("Motion Detected: ", state)
    };
  ...
  takePicture(options?: RequestPictureOptions): Promise<MediaObject> {
    ...
    this.console.debug("Pic: ", picture)
...
class EufyPlugin extends ScryptedDeviceBase implements DeviceProvider, Settings {
  ...
  private async initializeClient() {
    ...
    //this.log.a(this.client.getVersion());
    //this.console.debug(String(this.client.getConfig().enableEmbeddedPKCS1Support));
    
    this.client.on('tfa request', () => {
    
     //this.log.a(String(this.client.getConfig().enableEmbeddedPKCS1Support))
  ...
  private async deviceAdded(eufyDevice: eufy.Device) {
    if (!eufyDevice.isCamera) {
      ...
      this.console.debug("Battery Level: ", eufyDevice.getPropertyValue(PropertyName.DeviceBattery))
      return;
    }
    ...
    if (eufyDevice.hasBattery())
      ...
      this.console.debug("battery: ", eufyDevice.getPropertyValue("BatteryValue"))
